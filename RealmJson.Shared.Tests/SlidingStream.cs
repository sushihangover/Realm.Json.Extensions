...
namespace RealmJson.Test
{
	public class SlidingStream : Stream
	{
		...
		int _readTimeout = -1;
		readonly object _writeSyncRoot = new object();
		readonly object _readSyncRoot = new object();
		readonly LinkedList<ArraySegment<byte>> _pendingSegments = new LinkedList<ArraySegment<byte>>();
		readonly ManualResetEventSlim _dataAvailableResetEvent = new ManualResetEventSlim();
		...
		public override int Read(byte[] buffer, int offset, int count)
		{
			lock (_readSyncRoot)
			{
				if (_pendingSegments.Count == 0)
				{
					...
					_dataAvailableResetEvent.Wait(_readTimeout);
				}
